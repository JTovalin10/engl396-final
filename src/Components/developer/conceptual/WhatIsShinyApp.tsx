export default function WhatIsShinyApp() {
  return (
    <article className="endpoint-card">
      ...
      <p>Shiny is an R package from Posit that lets you build interactive web applications directly from R code, with no HTML, CSS, or JavaScript required. A Shiny app is structured around two things: a <strong>UI</strong> that defines what the user sees, and a <strong>server</strong> that defines what happens when they interact with it.</p>

      <h4>How Shiny Differs from a Regular R Script</h4>
      <p>In a standard R script, code runs top to bottom once and produces a fixed output. In a Shiny app, the server function contains <strong>reactive blocks</strong> that re-execute automatically whenever their inputs change. This means the application is always live, with outputs updating in real time without rerunning the whole script.</p>
      <p>For example, when a user picks a different artist in the Artist's Albums tab, Shiny detects that <code>input$band_select</code> changed and automatically re-runs the block that calls <code>albums_by_bands()</code>. No page reload, manual refresh, or extra wiring is required.</p>
      ...
      <p>In My Favorite Albums, this call lives in <code>app.R</code> after all libraries are loaded and all helper files are sourced.</p>
